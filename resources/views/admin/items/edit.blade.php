...
@section('content')
    <div class="flex justify-center py-10">
        <div class="w-full max-w-lg bg-white shadow-lg rounded-lg p-6">
            ...
            <form action="{{ route('admin.categories.items.update', [$category, $item]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea name="standard" id="standard" rows="4"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm
                 focus:border-yellow-500 focus:ring focus:ring-yellow-200 focus:ring-opacity-50">{{ old('standard', $item->standard) }}</textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="recommendation" id="recommendation" rows="4"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm
                 focus:border-yellow-500 focus:ring focus:ring-yellow-200 focus:ring-opacity-50">{{ old('recommendation', $item->recommendation) }}</textarea>
                </div>
